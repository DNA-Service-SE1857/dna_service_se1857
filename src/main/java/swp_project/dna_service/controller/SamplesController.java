...
public class SamplesController {
    ...
    public ApiResponse<List<SamplesResponse>> getSamplesByTestResultId(@PathVariable String testResultId) {
        log.info("Received get samples request for test result ID: {}", testResultId);
        List<SamplesResponse> responses = samplesService.getSamplesByTestResult(testResultId);
        return ApiResponse.<List<SamplesResponse>>builder()
                .code(200)
                .message("Lấy danh sách mẫu xét nghiệm theo kết quả thành công")
                .result(responses)
                .build();
    }
